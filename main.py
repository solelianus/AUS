...
def extract_info(
    url="https://search.studyaustralia.gov.au/course/associate-degree-of-engineering/d572f5ae0d48176138a53d5c0619b5a2",
):
    r = requests.get(url)
    soup = BeautifulSoup(r.text, "html.parser")
    app = soup.find("div", id="app")
    assert isinstance(app, Tag), "Expected a Tag object but found None."
    data_page = app.get("data-page")
    if isinstance(data_page, str):
        try:
        ...
        print("Unexpected list of strings for 'data-page'.")
        return
    else:
        print("No valid data-page attribute found.")
        return
    ...
    attendance_options = (
        f"{attendance_data.get('name', 'N/A')} - {attendance_data.get('length', 'N/A')} years"
        if attendance_data.get("name") and attendance_data.get("length")
        else "N/A"
    )
...
def main():
    ...
    for page_number in range(
        1, page_numbers + 1
    ):  # Loop through specified page numbers
